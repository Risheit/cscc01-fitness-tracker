...
Unit Testing Total Mark: 6 / 8

**Grader's Notes:**
- Missing unit tests for newly developed features (comments, workout history, times, and analytics page, etc.)
...
1. Workout scheduling
  - When a workout is created, you can not set times and dates for the workout.
  - Can view the workouts that are scheduled for each day of the week and time.
  - Can delete workouts from the schedule.
2. Data visualization
  - Users can keep track of their progression in terms of weights over time when theyr are doing the workouts.
  - Afterwards, they can see this data visualized in a graph for their review.
  - The data visualization was created using ReCharts.
  - In the database, there is a distinguishment between exercises that are weighted and those that are not.
  - New features to allow users to view their previously completed workouts and sort them in chornological order.
3. Added comments to workouts
  - Users can add comments to their own workouts and others can see them when
    a workout is shared with them.
  - People who have received a shared workout can also leave comments to the workout.
  - Comments can be liked and deleted.
  - How are you keeping track of whether or not a user has liked a comment or not?
    - There's a one to many relationship between Users and the Comments that they have liked tracked through
      a UsersLikedComments relation.
