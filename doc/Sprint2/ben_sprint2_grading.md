...
**Grader's Notes:** There are still a considerable amount of stories that are incomplete by the end of the current sprint, the team should take care to accurately estimate each story to avoid this in the future.
...
**Grader's Notes:** Architecture diagram is present but system decomposition is missing with detailed descriptions of components.
...
**Grader's Notes:** Following your README instructions, I ran `npm test` in your project's `go-pulse` folder and there was an error so I was unable to run the tests. If you feel like I missed something, let me know and we can discuss a regrade.
...
1. Conversation app
  a. Can send messages to users
  b. Realtime chat between clients using websockets
  c. Each message is saved in the database for persistent communication
  d. Future plans to include multi-media messages sending photos and videos
  e. Planned to send workouts too.
2. Workout builder
  a. UI is very pleasing
  b. Creating workouts has been implemented and data is stored in the database.
  c. API ninjas using a workout API
